...
class ModelHandler(BaseHandler):
    ...
    def initialize(self, context):
        ...
        self.map_location, self.device, self.use_gpu = (
            ("cuda", torch.device(f"cuda:{str(gpu_id)}"), True)
        ...
        if not os.path.exists(f"{model_dir}/models"):
            with zipfile.ZipFile(f"{model_dir}/{MODELSZIP}", "r") as zip_ref:
        ...
        state_dict = torch.load(
            f"{model_dir}/{CHECKPOINT}", map_location=self.map_location
        )
    ...
    def inference(self, preprocessed_data, *args, **kwargs):
        ...
        return torch.split(
            image_tensor,
            tuple(map(lambda d: d["number_of_images"], preprocessed_data)),
        )
